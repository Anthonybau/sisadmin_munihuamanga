...
class clsPersonaCapacitacion extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_capacitacion'; //nombre de la tabla
		$this->setKey='peca_id'; //campo clave
		...
		$this->destinoInsert='PersonaCapacitacion_lista.php';
		$this->destinoUpdate='PersonaCapacitacion_buscar.php';
		$this->destinoDelete='PersonaCapacitacion_lista.php';
		...
		$sql->addField("peca_actualfecha", 'NOW()', "String");
		$sql->addField("peca_actualusua", getSession("sis_userid"), "String");
                
                if ($_POST["hx_peca_valido"])
                    $sql->addField("peca_valido", 1, "Number");
                else
                    $sql->addField("peca_valido", 0, "Number");
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Tipo de Evento","8%", "C"); 
                        $otable->addColumnHeader("Fecha Doc.","6%", "C"); 
                        $otable->addColumnHeader("Instituci&oacute;n","34%", "C"); 
                        $otable->addColumnHeader("Curso","40%", "C"); 
                        $otable->addColumnHeader("T.Participaci&oacute;n","10%", "C"); 
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("peca_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("pecaid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaCapacitacion_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData($rs->field("tevento"));
                            $otable->addData(dtos($rs->field("peca_fechadocu")));
                            $otable->addData($rs->field("peca_organizador"));	
                            $otable->addData($rs->field("peca_nombrecurso"));	
                            $otable->addData($rs->field("tparticipante"));
...
class clsPersonaCapacitacion_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(peca_id::TEXT,5,'0') AS pecaid,  
                                      b.tabl_descripcion AS tevento,
                                      c.tabl_descripcion AS tparticipante,
                                      CASE WHEN a.peca_valido=1 THEN 'SI' ELSE 'NO' END AS acorde,
                                      i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres AS empleado,
                                      i.pers_dni AS dni,
                                      y.usua_login AS username,
                                      x.usua_login AS usernameactual
			FROM personal.persona_capacitacion a 
			LEFT JOIN catalogos.tabla b ON a.tabl_tevento = b.tabl_id  
			LEFT JOIN catalogos.tabla c ON a.tabl_tparticipante = c.tabl_id
                        LEFT JOIN personal.persona  i ON a.pers_id=i.pers_id
                        LEFT JOIN admin.usuario y ON a.usua_id=y.usua_id
                        LEFT JOIN admin.usuario x ON a.peca_actualusua=x.usua_id
	...
	function whereID($id){
		$this->addWhere("a.peca_id=$id");
	}
                
	function whereDescrip($descrip){
		$this->addWhere("(a.peca_organizador ILIKE '%$descrip%' OR a.peca_nombrecurso ILIKE '%$descrip%' OR a.peca_lugar ILIKE '%$descrip%')");
	}

        function wherePeriodo($fechIni,$fechFin){
		$this->addWhere("(a.peca_fechadocu>='$fechIni' AND a.peca_fechadocu<='$fechFin')");
	}
        
        function whereTipoEvento($tabl_tevento){
		$this->addWhere("a.tabl_tevento=$tabl_tevento");
	}
        
        function whereTipoParticipante($tabl_tparticipante){
		$this->addWhere("a.tabl_tparticipante=$tabl_tparticipante");
	}
                
        function whereAcorde($peca_valido){
		$this->addWhere("a.peca_valido=$peca_valido");
	}
        
	function orderUno(){
		$this->addOrder("a.peca_id DESC");
	}

        function orderDos(){
		$this->addOrder("i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres,a.peca_fechadocu");
	}
}
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsPersonaCapacitacion();
