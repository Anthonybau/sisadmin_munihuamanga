...
class clsPersonaLicencias extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_licencias'; //nombre de la tabla
		$this->setKey='peli_id'; //campo clave
		...
		$this->destinoInsert='PersonaLicencias_lista.php';
		$this->destinoUpdate='PersonaLicencias_buscar.php';
		$this->destinoDelete='PersonaLicencias_lista.php';
		...
		$sql->addField("peli_actualfecha", 'NOW()', "String");
		$sql->addField("peli_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Fecha Doc","10%");
                        $otable->addColumnHeader("Documento","35%");
                        $otable->addColumnHeader("Tipo","20%");
                        $otable->addColumnHeader("Remunerado","5%");
                        $otable->addColumnHeader("Desde","10%");
                        $otable->addColumnHeader("Hasta","10%");
                        $otable->addColumnHeader("Dias","5%");
                        $otable->addColumnHeader("Subv(D&iacute;as)","5%");
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("peli_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("peliid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaLicencias_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData(dtos($rs->field("peli_fecha")));	                            
                            $otable->addData($rs->field("peli_documento"));	
                            $otable->addData($rs->field("tipo_licencia"));	
                            $otable->addData($rs->field("remunerado"));
                            $otable->addData(dtos($rs->field("peli_desde")));
                            $otable->addData(dtos($rs->field("peli_hasta")));	
                            $otable->addData($rs->field("peli_dias"),"C");	
                            $otable->addData($rs->field("peli_subvencion"),"C");	
                            
                            if($rs->field("peli_estado")==9){
	...
	function eliminar(){
		...
		/* Sql a ejecutar */
		$sqlCommand="UPDATE $this->setTable SET peli_estado=CASE WHEN peli_estado=1 THEN 9 ELSE 1 END, ";
                $sqlCommand.=" peli_anulausua=".getSession("sis_userid").",peli_anulafecha=NOW() ";
...
class clsPersonaLicencias_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(a.peli_id::TEXT,5,'0') AS peliid,  
                                      b.tabl_descripcion AS tipo_licencia,
                                      CASE WHEN a.peli_remunerado=1 THEN 'SIN GOCE' ELSE 'CON GOCE' END AS remunerado,
                                      i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres AS empleado,
                                      i.pers_dni AS dni,
                                      y.usua_login AS username,
                                      x.usua_login AS usernameactual
			FROM personal.persona_licencias a 
                        LEFT JOIN catalogos.tabla   b ON a.tabl_tipo_licencia = b.tabl_id  
                        LEFT JOIN personal.persona  i ON a.pers_id=i.pers_id
                        LEFT JOIN admin.usuario y ON a.usua_id=y.usua_id
                        LEFT JOIN admin.usuario x ON a.peli_actualusua=x.usua_id
                        ";
	}
	...
	function whereID($id){
		$this->addWhere("a.peli_id=$id");
	}
        
	function whereDescrip($descrip){
		$this->addWhere("(a.peli_documento ILIKE '%$descrip%' OR peli_emisor ILIKE '%$descrip%')");
	}

        function whereEstado($estado){
		$this->addWhere("a.peli_estado=$estado");
	}
        
        function wherePeriodo($fechIni,$fechFin){
		$this->addWhere("(a.peli_fecha>='$fechIni' AND a.peli_fecha<='$fechFin')");
	}
        
        function wherePeriodovsPeriodo($fechIni,$fechFin){
		$this->addWhere("(a.peli_desde<='$fechFin' AND COALESCE(a.peli_hasta,'$fechIni'::DATE)>='$fechIni')");
	}
        
        function whereTipoLicencia($tabl_tipo_licencia){
		$this->addWhere("a.tabl_tipo_licencia=$tabl_tipo_licencia");
	}
        
        function whereTipoRemuneado($tipo_remunerado){
		$this->addWhere("a.peli_remunerado=$tipo_remunerado");
	}
        
	function orderUno(){
		$this->addOrder("a.peli_id DESC");
	}
        
        function orderDos(){
		$this->addOrder("i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres,a.peli_fecha");
	}
