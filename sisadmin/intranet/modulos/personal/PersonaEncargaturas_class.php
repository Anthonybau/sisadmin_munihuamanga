...
class clsPersonaEncargaturas extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_encargaturas'; //nombre de la tabla
		$this->setKey='peen_id'; //campo clave
		...
		$this->destinoInsert='PersonaEncargaturas_lista.php';
		$this->destinoUpdate='PersonaEncargaturas_buscar.php';
		$this->destinoDelete='PersonaEncargaturas_lista.php';
        ...
        function getSql(){
		$sql=new clsPersonaEncargaturas_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}

        function addField(&$sql){
		$sql->addField("peen_actualfecha", 'NOW()', "String");
		$sql->addField("peen_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Tipo","8%", "C"); 
                        $otable->addColumnHeader("Remunerado","6%");                        
                        $otable->addColumnHeader("Dependencia","19%");
                        $otable->addColumnHeader("","1%");
                        $otable->addColumnHeader("Cargo","16%");
                        $otable->addColumnHeader("Fecha Doc","8%");
                        $otable->addColumnHeader("Documento","20%");
                        $otable->addColumnHeader("Desde","10%");
                        $otable->addColumnHeader("Hasta","10%");
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("peen_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("peenid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaEncargaturas_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData($rs->field("tipo"));
                            $otable->addData($rs->field("remunerado"));
                            ...
                            if($rs->field("pero_id")){
                                ...
                                $buttonX = new Button;
                                $buttonX->addItem("Desplazar","javascript:if(confirm('Seguro de Desplazar?')){xajax_desplazar('$id')}","content",2,0,"botao");	
                                $otable->addData($buttonX->writeHTML());
                            }
                            
                            $otable->addData($rs->field("peen_cargo"));
                            $otable->addData(dtos($rs->field("peen_fecha")));	                            
                            $otable->addData($rs->field("peen_documento"));
                            $otable->addData(dtos($rs->field("peen_desde")));
                            $otable->addData(dtos($rs->field("peen_hasta")));	
                            
                            if($rs->field("peen_estado")==9){
                                $otable->addRow('ANULADO'); // adiciona linea
                            }elseif($rs->field("peen_desde") && !$rs->field("peen_hasta")){
	...
	function eliminar(){
		...
		/* Sql a ejecutar */
		$sqlCommand="UPDATE $this->setTable SET peen_estado=CASE WHEN peen_estado=1 THEN 9 ELSE 1 END, ";
                $sqlCommand.=" peen_anulausua=".getSession("sis_userid").",peen_anulafecha=NOW() ";
...
class clsPersonaEncargaturas_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(a.peen_id::TEXT,5,'0') AS peenid,  
                                      CASE WHEN a.peen_tipo=1       THEN 'DESIGNACION' ELSE 'ENCARGATURA' END AS tipo,
                                      CASE WHEN a.peen_remunerado=1 THEN 'SIN GOCE'  ELSE 'CON GOCE'  END AS remunerado,
                                      b.depe_nombre AS dependencia,
                                      c.pero_id,
                                      i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres AS empleado,
                                      i.pers_dni AS dni,
                                      y.usua_login AS username,
                                      x.usua_login AS usernameactual
			FROM personal.persona_encargaturas a 
                        LEFT JOIN catalogos.dependencia b ON a.depe_id=b.depe_id
                        LEFT JOIN personal.persona_rotaciones c ON a.peen_id=c.peen_id                        
                        LEFT JOIN personal.persona  i ON a.pers_id=i.pers_id
                        LEFT JOIN admin.usuario y ON a.usua_id=y.usua_id
                        LEFT JOIN admin.usuario x ON a.peen_actualusua=x.usua_id
                        ";
	}
	...
	function wherePadreID($padre_id){
        ...
        function whereTipo($peen_tipo){
		$this->addWhere("a.peen_tipo=$peen_tipo");
	}
        
	function whereID($id){
		$this->addWhere("a.peen_id=$id");
	}
	...
	function whereDescrip($descrip){
		$this->addWhere("a. peen_documento ILIKE '%$descrip%'");
	}

        function whereEstado($estado){
		$this->addWhere("a.peen_estado=$estado");
	}
        
        function wherePeriodo($fechIni,$fechFin){
		$this->addWhere("(a.peen_fecha>='$fechIni' AND a.peen_fecha<='$fechFin')");
	}
        
        function wherePeriodovsPeriodo($fechIni,$fechFin){
		$this->addWhere("(a.peen_desde<='$fechFin' AND COALESCE(a.peen_hasta,'$fechIni'::DATE)>='$fechIni')");
	}
        
	function orderUno(){
		$this->addOrder("a.peen_id DESC");
	}
        
        function orderDos(){
		$this->addOrder("i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres,a.peen_tipo,a.peen_fecha");
	}
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsPersonaEncargaturas();
