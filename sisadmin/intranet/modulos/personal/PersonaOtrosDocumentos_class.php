...
class clsPersonaOtrosDocumentos extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_otros_documentos'; //nombre de la tabla
		$this->setKey='peod_id'; //campo clave
		...
		$this->destinoInsert='PersonaOtrosDocumentos_lista.php';
		$this->destinoUpdate='PersonaOtrosDocumentos_buscar.php';
		$this->destinoDelete='PersonaOtrosDocumentos_lista.php';
		...
		$sql->addField("peod_actualfecha", 'NOW()', "String");
		$sql->addField("peod_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Tipo","38%", "C");                         
                        $otable->addColumnHeader("Fecha","10%", "C"); 
                        $otable->addColumnHeader("Documento","10%", "C"); 
                        $otable->addColumnHeader("Contenido","40%", "C");
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("peod_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("peodid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaOtrosDocumentos_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData($rs->field("tipo_documento"));	                            
                            $otable->addData(dtos($rs->field("peod_fecha")));	
                            $otable->addData($rs->field("peod_documento"));	
                            $otable->addData($rs->field("peod_contenido"));
...
class clsPersonaOtrosDocumentos_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(a.peod_id::TEXT,5,'0') AS peodid,  
			...
			FROM personal.persona_otros_documentos a 
                        LEFT JOIN catalogos.tabla   b ON a.tabl_tipo_documento=b.tabl_id  
                        LEFT JOIN personal.persona  i ON a.pers_id=i.pers_id
                        LEFT JOIN admin.usuario     y ON a.usua_id=y.usua_id 
                        LEFT JOIN admin.usuario     x ON a.peod_actualusua=x.usua_id
                        ";
	}
	...
	function whereID($id){
		$this->addWhere("a.peod_id=$id");
	}
        
	function whereDescrip($descrip){
		$this->addWhere("a. peod_contenido ILIKE '%$descrip%'");
	}

	function orderUno(){
		$this->addOrder("a.peod_id DESC");
	}
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsPersonaOtrosDocumentos();
