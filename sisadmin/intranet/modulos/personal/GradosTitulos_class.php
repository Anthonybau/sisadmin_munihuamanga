...
class GradoTitulo extends entidad {

    function __construct($id='',$title='') {
        $this->setTable='personal.grado_estudio'; //nombre de la tabla
        $this->setKey='gres_id'; //campo clave
        ...
        $this->return_txt = strtoupper(getParam('Sr_gres_descripcion')); /* Texo que se devuelve */
    ...
    function getSql(){
        $sql=new GradoTitulo_SQLlista();
        $sql->whereID($this->id);
        return($sql->getSQL());
    }

    function addField(&$sql){
        $sql->addField("gres_actualfecha", 'NOW()', "String");
	$sql->addField("gres_actualusua", getSession("sis_userid"), "String");
    }    
    ...
    function buscar($op,$formData,$arrayParam,$pg=1,$Nameobj='') {
        ...
        if(strlen($cadena)>0 or $busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
            ...
            if ($rs->numrows()>0) {
                ...
                while ($rs->getrow()) {
                    $id = $rs->field("gres_id"); // captura la clave primaria del recordsource
                    $campoTexto_de_Retorno = especialChar($rs->field("gres_descripcion"));

                    if ($nomeCampoForm) { /* si la llamada es desde la busqueda avanzada (AvanzLookup) */
                        $otable->addData(addLink($rs->field("gres_descripcion"),"javascript:update('$id','$campoTexto_de_Retorno',$numForm)","Click aqu&iacute; para seleccionar el registro"));
                        ...
                        $otable->addData(addLink($campoTexto_de_Retorno,"GradosTitulos_edicion.php?id=$id&".$param->buildPars(false),"Click aqu&iacute; para consultar o editar el registro"));
                    }
    ...
    function buscarGradoTitulo($op,$cadena,$colSearch,$colOrden=1,$Nameobj,$accion=1)
    {
            global $conn;
    ...
    //	  $objResponse->addAlert($sql);
                            ...
                            if ($rs->numrows()>0) {
                                            ...
                                            $otable->addColumnHeader("Grado o T&iacute;tulo","98%", "L"); 
                                            ...
                                            while ($rs->getrow()) {
                                                    ...
                                                    $button->setDiv(FALSE);
                                                    $button->setStyle("");
                                                    $button->addItem("Aceptar","javascript:xajax_eligeGradoTitulo($id,'$campoTexto_de_Retorno','$accion')","content",2,0,"botonAgg","button","","btn_$id");
                                                    $otable->addData($button->writeHTML());	

                                                    $otable->addData($rs->field("gres_descripcion"));
                                                    if($accion==1){
                                                        $otable->addData("<a class=\"link\" href=\"#\" onClick=\"javascript:modiGradoTitulo($id)\"><img src=\"../../img/editar.gif\" border=0 align=absmiddle hspace=1 alt=\"Editar\"></a>");
                                                    }
                                                    else {
                                                           $otable->addData("&nbsp;");
                                                    }
    ...
    function guardar(){
            ...
            if($nomeCampoForm){//si se llama desde una ventana emergente (avanzlookup) para seleccionar un valor
                    ...
                    echo "<script language=\"javascript\">
                                    ...
                                    parent.opener.parent.content.document.getElementById('divResultado2').innerHTML = '';
                                    parent.parent.close();
                            </script>";
...
class GradoTitulo_SQLlista extends selectSQL {

    function __construct() {
        $this->sql="SELECT a.*,
                           c.usua_login AS username,
                           d.usua_login AS usernameactual
                           FROM personal.grado_estudio a 
                           LEFT JOIN usuario c ON a.usua_id=c.usua_id
                           LEFT JOIN usuario d ON a.gres_actualusua=d.usua_id                           
                    ";
        
    }
    function whereID($id) {
        $this->addWhere("a.gres_id=$id");
    }

    function whereDescrip($descrip) {
        if($descrip) $this->addWhere("(a.gres_descripcion ILIKE '%$descrip%')");
    }

    function orderUno() {
        $this->addOrder("a.gres_id DESC");
    }
}
