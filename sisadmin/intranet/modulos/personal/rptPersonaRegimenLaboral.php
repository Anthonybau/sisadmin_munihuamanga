...
class Reporte extends GenReporte
{
	...
	function Cabecera(){
		...
		$this->Cell(15,$this->lineHeight+1,'FECHA',1,0,'C',1);		
                $this->Cell(50,$this->lineHeight+1,'REGIMEN LABORAL',1,0,'C',1);		
                $this->Cell(50,$this->lineHeight+1,'CONDICION',1,0,'C',1);		
                $this->Cell(50,$this->lineHeight+1,'DOCUMENTO',1,0,'C',1);		
	...
	function SeteoCampos(){
		...
		$this->beginBlock(); 		
                $this->printField(utf8_decode('TRABAJADOR: ').$rs->field("empleado")." / ".$rs->field("dni"), 'HG1','bold',0,'L');
                $cont_empleado++;
	}        
		
	function Detalle(){
		global $rs;	
                $this->printField(dtos($rs->field("perl_fecha")), 'C1','items',0,'L');
		$this->printField($rs->field("rela_descripcion"), 'C2','items',0,'L');		
                $this->printField($rs->field("sit_laboral"), 'C3','items',0,'L');		
                $this->printField($rs->field("perl_documento"), 'C4','items',0,'L');		
...
/*	recibo los parametros */
	...
	$_titulo = "REGISTROS DE REGIMEN LABORAL" ; // T?tulo del reporte
