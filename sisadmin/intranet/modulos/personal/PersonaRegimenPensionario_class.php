...
class clsPersonaRegimenPensionario extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_regimen_pensionario'; //nombre de la tabla
		$this->setKey='perp_id'; //campo clave
		...
		$this->destinoInsert='PersonaRegimenPensionario_lista.php';
		$this->destinoUpdate='PersonaRegimenPensionario_buscar.php';
		$this->destinoDelete='PersonaRegimenPensionario_lista.php';
		...
		$sql->addField("perp_actualfecha", 'NOW()', "String");
		$sql->addField("perp_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("R&eacute;gimen Pensionario","28%", "C"); 
                        $otable->addColumnHeader("Fecha Afilia","10%", "C");                         
                        $otable->addColumnHeader("AFP","40%", "C"); 
                        $otable->addColumnHeader("Tipo Comisi&oacute;n","20%", "C");                         
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("perp_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("perpid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaRegimenPensionario_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData($rs->field("repe_descripcion"));	
                            $otable->addData(dtos($rs->field("perp_afpafiliacion")));	                            
                            $otable->addData($rs->field("afp_nombre"));
                            $otable->addData($rs->field("tipo_comision"));
                            
                            if($rs->field("perp_estado")==9){
	...
	function eliminar(){
		...
		/* Sql a ejecutar */
		$sqlCommand="UPDATE $this->setTable SET perp_estado=CASE WHEN perp_estado=1 THEN 9 ELSE 1 END, ";
                $sqlCommand.=" perp_anulausua=".getSession("sis_userid").",perp_anulafecha=NOW() ";
...
class clsPersonaRegimenPensionario_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(a.perp_id::TEXT,5,'0') AS perpid,  
                                      b.repe_descripcion,
                                      c.afp_nombre,
                                      d.tabl_descripcion AS tipo_comision,
                                      i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres AS empleado,
                                      i.pers_dni AS dni,
                                      y.usua_login AS username,
                                      x.usua_login AS usernameactual
			FROM personal.persona_regimen_pensionario   a 
                        LEFT JOIN catalogos.regimen_pensionario     b ON a.repe_id = b.repe_id  
                        LEFT JOIN catalogos.afp                     c ON a.afp_id = c.afp_id  
                        LEFT JOIN catalogos.tabla                   d ON a.tabl_tipocomision = d.tabl_id  
                        LEFT JOIN personal.persona                  i ON a.pers_id=i.pers_id
                        LEFT JOIN admin.usuario                     y ON a.usua_id=y.usua_id
                        LEFT JOIN admin.usuario                     x ON a.perp_actualusua=x.usua_id
                        ";
	}
	...
	function whereID($id){
		$this->addWhere("a.perp_id=$id");
	}
        
	function whereDescrip($descrip){
		$this->addWhere("a.perp_documento ILIKE '%$descrip%'");
	}

        function whereEstado($estado){
		$this->addWhere("a.perp_estado=$estado");
	}
        
	function orderUno(){
		$this->addOrder("a.perp_id DESC");
	}
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsPersonaRegimenPensionario();
