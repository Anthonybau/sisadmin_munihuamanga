...
class clsPersonaRotaciones extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_rotaciones'; //nombre de la tabla
		$this->setKey='pero_id'; //campo clave
		...
		$this->destinoInsert='PersonaRotaciones_lista.php';
		$this->destinoUpdate='PersonaRotaciones_buscar.php';
		$this->destinoDelete='PersonaRotaciones_lista.php';
        ...
        function getSql(){
		$sql=new clsPersonaRotaciones_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}

        function addField(&$sql){
		$sql->addField("pero_actualfecha", 'NOW()', "String");
		$sql->addField("pero_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Documento","25%", "C"); 
                        $otable->addColumnHeader("Dependencia","35%", "C");
                        $otable->addColumnHeader("Cargo Funcional","17%", "C");
                        $otable->addColumnHeader("Desde","5%", "C");
                        $otable->addColumnHeader("Hasta","5%", "C");
                        $otable->addColumnHeader("O","1%", "C","title=Origen");
                        $otable->addRowHead();
                        ...
                        while ($rs->getrow()) {
                            ...
                            $otable->addData(addLink($rs->field("peroid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaRotaciones_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData(dtos($rs->field("pero_fecha")));	
                            $otable->addData($rs->field("pero_documento"));	
                            $otable->addData($rs->field("dependencia"));
                            $otable->addData($rs->field("pero_cargofuncional"));
                            $otable->addData(dtos($rs->field("pero_desde")));	
                            $otable->addData(dtos($rs->field("pero_hasta")));	
                            if($rs->field("pero_modo_ingreso")==1){
                                $otable->addData("D","C","","DIRECTA");
                            }else{
                                $otable->addData("E","C","","DESDE ENCARGATURAS");
                            }
	...
	function guardar(){
		global $conn,$param;
                ...
                if(SIS_GESTDOC==1){
                    require_once("../gestdoc/setAcumDespacho_class.php");

                    $pers_id=getParam("nx_pers_id");
                    $userid= getDbValue("SELECT usua_id
                                                FROM admin.usuario
                                                        WHERE pdla_id IN (SELECT pdla_id FROM personal.persona_datos_laborales
                                                WHERE pers_id=$pers_id)");

                    $setNotiEnProcesoUsuario=new setNotiDespachosEnProcesoUsuario($pers_id);
                    $setNotiEnProcesoUsuario->whereUserID($userid);
                    $sSql=$setNotiEnProcesoUsuario->getSQL();

                    $setNotiPorRecibirUsuario=new setNotiDespachosPorRecibirUsuario($pers_id);
                    $setNotiPorRecibirUsuario->whereUserID($userid);
                    $sSql.=";".$setNotiPorRecibirUsuario->getSQL();    
...
class clsPersonaRotaciones_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(a.pero_id::TEXT,5,'0') AS peroid,  
			...
			FROM personal.persona_rotaciones a 
                        ...
                        LEFT JOIN admin.usuario             x ON a.pero_actualusua=x.usua_id
                        ";
	}
	...
	function whereID($id){
		$this->addWhere("a.pero_id=$id");
	}
        
        function wherePeriodo($fechIni,$fechFin){
		$this->addWhere("(a.pero_fegresado>='$fechIni' AND a.pero_fegresado<='$fechFin')");
	}
        
	function whereDescrip($descrip){
		$this->addWhere("a.pero_documento ILIKE '%$descrip%'");
	}

	function orderUno(){
		$this->addOrder("a.pero_id DESC");
	}

        function orderDos(){
		$this->addOrder("i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres,a.pero_fecha");
	}
}
