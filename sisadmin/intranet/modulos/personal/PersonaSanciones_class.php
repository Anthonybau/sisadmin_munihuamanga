...
class clsPersonaSanciones extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_sanciones'; //nombre de la tabla
		$this->setKey='pesa_id'; //campo clave
		...
		$this->destinoInsert='PersonaSanciones_lista.php';
		$this->destinoUpdate='PersonaSanciones_buscar.php';
		$this->destinoDelete='PersonaSanciones_lista.php';
		...
		$sql->addField("pesa_actualfecha", 'NOW()', "String");
		$sql->addField("pesa_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Destituci&oacute;n","10%");                        
                        $otable->addColumnHeader("Fecha Doc","10%");
                        $otable->addColumnHeader("Documento","58%");
                        $otable->addColumnHeader("Desde","10%");
                        $otable->addColumnHeader("Hasta","10%");
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("pesa_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("pesaid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaSanciones_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData($rs->field("destitucion"));                            
                            $otable->addData(dtos($rs->field("pesa_fecha")));	                            
                            $otable->addData($rs->field("pesa_documento"));	
                            $otable->addData(dtos($rs->field("pesa_desde")));
                            $otable->addData(dtos($rs->field("pesa_hasta")));	
...
class clsPersonaSanciones_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(a.pesa_id::TEXT,5,'0') AS pesaid,  
                                      CASE WHEN a.pesa_destitucion=1 THEN 'SIN DESTITUCION' ELSE 'CON DESTITUCION' END AS destitucion,
                                      i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres AS empleado,
                                      i.pers_dni AS dni,
                                      y.usua_login AS username,
                                      x.usua_login AS usernameactual
			FROM personal.persona_sanciones a 
                        LEFT JOIN personal.persona  i ON a.pers_id=i.pers_id
                        LEFT JOIN admin.usuario     y ON a.usua_id=y.usua_id
                        LEFT JOIN admin.usuario     x ON a.pesa_actualusua=x.usua_id
                        ";
	}
	...
	function whereID($id){
		$this->addWhere("a.pesa_id=$id");
	}
        
        function whereTipoDestitucion($pesa_destitucion){
		$this->addWhere("a.pesa_destitucion=$pesa_destitucion");
	}
        
        function whereFiltro($filtro){
		$this->addWhere("(a.pesa_documento ILIKE '%$filtro%' OR a.pesa_motivo ILIKE '%$filtro%')");
	}
        
	function whereDescrip($descrip){
		$this->addWhere("a. pesa_documento ILIKE '%$descrip%'");
	}

        function wherePeriodo($fechIni,$fechFin){
		$this->addWhere("(a.pesa_fecha>='$fechIni' AND a.pesa_fecha<='$fechFin')");
	}
        
        function wherePeriodovsPeriodo($fechIni,$fechFin){
		$this->addWhere("(a.pesa_desde<='$fechFin' AND COALESCE(a.pesa_hasta,'$fechIni'::DATE)>='$fechIni')");
	}
        
	function orderUno(){
		$this->addOrder("a.pesa_id DESC");
	}
        
        function orderDos(){
		$this->addOrder("i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres,a.pesa_fecha");
	}
