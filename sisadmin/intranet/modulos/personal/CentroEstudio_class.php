...
class CentroEstudio extends entidad {

    function __construct($id='',$title='') {
        $this->setTable='personal.centro_estudio'; //nombre de la tabla
        $this->setKey='cees_id'; //campo clave
        ...
        $this->return_val = getParam('Sr_cees_ruc'); /* Valor que se devuelve */
        $this->return_txt = strtoupper(getParam('Sr_cees_nombre')); /* Texo que se devuelve */
    ...
    function getSql(){
        $sql=new CentroEstudio_SQLlista();
	$sql->whereID($this->id);
	return($sql->getSQL());
    }
    ...
    function addField(&$sql){
		$sql->addField("cees_actualfecha", 'NOW()', "String");
		$sql->addField("cees_actualusua", getSession("sis_userid"), "String");
    }
    ...
    function buscar($op,$formData,$arrayParam,$pg=1,$Nameobj='') {
		...
		$otable->addColumnHeader("C&oacute;digo","1%", "L"); 
                $otable->addColumnHeader("RUC","10%", "L"); 
                $otable->addColumnHeader("Raz&oacute;n Social","79%", "L"); 
                $otable->addColumnHeader("Clase","10%", "L"); 

                $otable->addRowHead();

                while ($rs->getrow()) {
                    $id = $rs->field("cees_id"); // captura la clave primaria del recordsource
                    $campoTexto_de_Retorno = especialChar($rs->field("cees_nombre"));

                    if ($nomeCampoForm) { /* si la llamada es desde la busqueda avanzada (AvanzLookup) */
                        $otable->addData(addLink($rs->field("cees_nombre"),"javascript:update('$id','$campoTexto_de_Retorno',$numForm)","Click aqu&iacute; para seleccionar el registro"));
                        ...
                        $otable->addData(addLink($rs->field("cees_id"),"CentroEstudio_edicion.php?id=$id&clear=1&".$param->buildPars(false),"Click aqu&iacute; para consultar o editar el registro"));
                        $otable->addData($rs->field("cees_ruc"));
                        $otable->addData($rs->field("cees_nombre"));
    ...
    function buscarCentroEstudio($op,$cadena,$colSearch,$colOrden=1,$Nameobj,$accion=1,$tabl_etapa_proceso=1)
    {
            global $conn;
    ...
    //	  $objResponse->addAlert($sql);
                            ...
                            if ($rs->numrows()>0) {
                                            ...
                                            $otable->addColumnHeader("RUC","8%", "L");
                                            $otable->addColumnHeader("Raz&oacute;n Social","89%", "L"); 
                                            $otable->addColumnHeader("Clase","1%", "L");
                                            $otable->addColumnHeader("",false,"1%","C"); 

                                            $otable->addRow(); // adiciona la linea (TR)
                                            while ($rs->getrow()) {
                                                    $id = $rs->field("cees_id");// captura la clave primaria del recordsource
                                                    $cees_ruc = $rs->field("cees_ruc");// 
                                                    $campoTexto_de_Retorno = especialChar($rs->field("cees_nombre"));

                                                    $button = new Button;
                                                    $button->setDiv(FALSE);
                                                    $button->setStyle("");
                                                    $button->addItem("Aceptar","javascript:xajax_eligeCentroEstudio($id,'$campoTexto_de_Retorno','$cees_ruc','$accion')","content",2,0,"botonAgg","button","","btn_$id");
                                                    ...
                                                    $otable->addData($rs->field("clase"));
                                                    if($accion==1){
                                                        $otable->addData("<a class=\"link\" href=\"#\" onClick=\"javascript:modiCentroEstudio($id)\"><img src=\"../../img/editar.gif\" border=0 align=absmiddle hspace=1 alt=\"Editar\"></a>");
                                                    }
                                                    else {
                                                           $otable->addData("&nbsp;");
                                                    }
    ...
    function guardar(){
            ...
            if($nomeCampoForm){//si se llama desde una ventana emergente (avanzlookup) para seleccionar un valor
                    ...
                    echo "<script language=\"javascript\">
                                    ...
                                    parent.opener.parent.content.document.forms[0].$nomeCampoForm.value = '$return_val';
                                    parent.opener.parent.content.document.forms[0].tr_cees_id.value = ".$padre_id.";    
                                    parent.opener.parent.content.document.getElementById('divResultado').innerHTML = '';
                                    parent.parent.close();
                            </script>";
...
class CentroEstudio_SQLlista extends selectSQL {

    function __construct() {
        $this->sql="SELECT a.*,
                            CASE WHEN a.cees_tipo=1 THEN 'PUBLICA' ELSE 'PRIVADA' END AS clase,
                            c.usua_login AS username,
                            d.usua_login AS usernameactual
                            FROM personal.centro_estudio a
                            LEFT JOIN admin.usuario c ON a.usua_id=c.usua_id
                            LEFT JOIN admin.usuario d ON a.cees_actualusua=d.usua_id                            
                        ";
    }
    function whereID($id) {
        $this->addWhere("a.cees_id='$id'");
    }

    function whereCodigo($codigo) {
        $this->addWhere("a.cees_ruc='$codigo'");
    }    
    function whereDescrip($descrip) {
        if($descrip) $this->addWhere("a.cees_nombre ILIKE '%$descrip%'");
    }

    function orderUno() {
        $this->addOrder("a.cees_id DESC");
    }
