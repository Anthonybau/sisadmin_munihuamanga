...
class clsPersonaFamilia extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_familia'; //nombre de la tabla
		$this->setKey='pefa_id'; //campo clave
		...
		$this->destinoInsert='PersonaFamilia_lista.php';
		$this->destinoUpdate='PersonaFamilia_buscar.php';
		$this->destinoDelete='PersonaFamilia_lista.php';
		...
		$sql->addField("pefa_actualfecha", 'NOW()', "String");
		$sql->addField("pefa_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Apellidos","40%", "C"); 
                        $otable->addColumnHeader("Nombres","40%", "C"); 
                        $otable->addColumnHeader("DNI","7%", "C"); 
                        $otable->addColumnHeader("Sexo","1%", "C");
                        $otable->addColumnHeader("Parentesco","10%", "C");
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("pefa_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("pefaid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaFamilia_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData($rs->field("pefa_apellpaterno").' '.$rs->field("pefa_apellmaterno"));	
                            $otable->addData($rs->field("pefa_nombres"));	
                            $otable->addData($rs->field("pefa_dni"));
                            $otable->addData($rs->field("pefa_sexo"));
                            $otable->addData($rs->field("parentesco"));                            
//                            $link='';
//                            $link.=$rs->field("pefa_adjunto1")?addLink($rs->field("pefa_adjunto1"),PATH_INC."auxiliar.php?pag=".PUBLICUPLOAD."/escalafon/".$rs->field("pefa_adjunto1"),"Click aqu&iacute; para ver Archivo","content","ls-modal"):'';
//                            $link.=$rs->field("pefa_adjunto1")?addLink("<br>".$rs->field("pefa_adjunto2"),PATH_INC."auxiliar.php?pag=".PUBLICUPLOAD."/escalafon/".$rs->field("pefa_adjunto2"),"Click aqu&iacute; para ver Archivo","content","ls-modal"):'';
//                            $otable->addData($link);
...
class clsPersonaFamilia_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(pefa_id::TEXT,5,'0') AS pefaid,  
                                      b.tabl_descripcion AS parentesco,
                                      c.tabl_descripcion AS estado_civil,
                                      i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres AS empleado,
                                      i.pers_dni AS dni,                                      
                                      y.usua_login AS username,
                                      x.usua_login AS usernameactual
			FROM persona_familia a 
			LEFT JOIN tabla             b ON a.tabl_idparentesco = b.tabl_id  
                        LEFT JOIN tabla             c ON a.tabl_idestadocivil = c.tabl_id  
                        LEFT JOIN personal.persona  i ON a.pers_id=i.pers_id
                        LEFT JOIN usuario y ON a.usua_id=y.usua_id
                        LEFT JOIN usuario x ON a.pefa_actualusua=x.usua_id
	...
	function whereID($id){
		$this->addWhere("a.pefa_id=$id");
	}
        
        
	function whereDescrip($descrip){
		$this->addWhere("(a.pefa_apellpaterno ILIKE '%$descrip%' 
                               OR a.pefa_apellmaterno ILIKE '%$descrip%'
                               OR a.pefa_nombres ILIKE '%$descrip%'
                               OR a.pefa_apellpaterno||' '||a.pefa_apellmaterno||' '||a.pefa_nombres ILIKE '%$descrip%'     
                                 )");
	}

	function orderUno(){
		$this->addOrder("a.pefa_id DESC");
	}
