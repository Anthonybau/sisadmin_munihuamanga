...
class Reporte extends GenReporte
{
	...
	function Cabecera(){
		...
		$this->Cell(10,$this->lineHeight+1,'COD',1,0,'C',1);		
                $this->Cell(40,$this->lineHeight+1,'CONCEPTO',1,0,'C',1);		
                $this->Cell(30,$this->lineHeight+1,'CATEGORIA',1,0,'C',1);		
                $this->Cell(15,$this->lineHeight+1,'ESPECIF',1,0,'C',1);		
                $this->Cell(30,$this->lineHeight+1,'JUDICIAL',1,0,'C',1);		
                $this->Cell(5,$this->lineHeight+1,'%',1,0,'C',1);		
                $this->Cell(10,$this->lineHeight+1,'IMP',1,0,'C',1);		                
                $this->Cell(40,$this->lineHeight+1,'TIPO PLANILLA',1,0,'C',1);
                $this->Cell(8,$this->lineHeight+1,'EST',1,0,'C',1);
	...
	function Detalle(){
		global $rs;	
		$this->printField($rs->field("cod_concepto"), 'C1','items',0,'L');		
                $this->printField($rs->field("concepto"), 'C2','items',0,'L');		
                $this->printField($rs->field("categoria_concepto"), 'C3','items',0,'L');
                $this->printField($rs->field("clas_id"), 'C4','items',0,'L');
                $this->printField($rs->field("judicial"), 'C5','items',0,'L');
                $this->printField($rs->field("pppa_porcentaje"), 'C6','items',0,'L');
                $this->printField(number_format($rs->field("pppa_importe"),2,'.',','), 'C7','items',0,'L');
                $this->printField($rs->field("tipo_planilla"), 'C8','items',0,'L');
                $this->printField(substr($rs->field("estado"),0,3), 'C9','items',0,'L');
...
/*	recibo los parametros */
	...
	$_titulo = "PERFIL DE PAGOS" ; // T?tulo del reporte
