...
class Reporte extends GenReporte
{
	...
	function Cabecera(){
		$this->SetX($this->blockPosX);
                ...
                $this->Cell(60,$this->lineHeight,'DEPENDENCIA','TRL',0,'C',1);		
		$this->Cell(20,$this->lineHeight,'FECHA','TRL',0,'C',1);		
                $this->Cell(50,$this->lineHeight,'DOCUMENTO','TRL',0,'C',1);		
                $this->Cell(40,$this->lineHeight,'PERIODO','1',1,'C',1);		
                
                $this->Cell(22,$this->lineHeight,'','BRL',0,'C',1);		
                $this->Cell(60,$this->lineHeight,'ENCARGADA','BRL',0,'C',1);		
		$this->Cell(20,$this->lineHeight,'DOC.','BRL',0,'C',1);		
                $this->Cell(50,$this->lineHeight,'','BRL',0,'C',1);		
	...
	function SeteoCampos(){
		...
		$this->addField('C1',  99999,	0,  22);
                $this->addField('C2',  99999,	0,  60);
		...
		global $rs;
		$this->beginBlock(); 		
                $this->printField($rs->field("tipo"), 'HG1','bold',0,'L');
	}
        
	function Detalle(){
		global $rs;	
		$this->printField($rs->field("remunerado"), 'C1','items',0,'L');		
                $this->printField($rs->field("dependencia"), 'C2','items',0,'L',true);		
                $this->printField(dtos($rs->field("peen_fecha")), 'C3','items',0,'C');		
                $this->printField($rs->field("peen_documento"), 'C4','items',0,'L',true);
                $this->printField(dtos($rs->field("peen_desde")), 'C5','items',0,'C');		
                $this->printField(dtos($rs->field("peen_hasta")), 'C6','items',0,'C');		
...
/*	recibo los parametros */ 
	...
	$_titulo = "REGISTRO DE DESIGNACIONES/ENCARGATURAS" ; // T?tulo del reporte
	
        $id_relacion = getParam("id_relacion"); 
        $peen_tipo = getParam("xr_peen_tipo"); 
        $fechIni = getParam("Dr_fechadesde"); 
        $fechFin = getParam("Dr_fechahasta"); 
        $depe_id = getParam("tx_depe_id"); 
        $cont_empleado=0;
/*	establecer conexion con la BD */
	...
	$sql=new clsPersonaEncargaturas_SQLlista();
        ...
        if(inlist($peen_tipo,'1,2')){//1->DESIGNACION, 2->ENCARGATURA
            $sql->whereTipo($peen_tipo);
        }
