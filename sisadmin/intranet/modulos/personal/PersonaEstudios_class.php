...
class clsPersonaEstudios extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_estudios'; //nombre de la tabla
		$this->setKey='pees_id'; //campo clave
		...
		$this->destinoInsert='PersonaEstudios_lista.php';
		$this->destinoUpdate='PersonaEstudios_buscar.php';
		$this->destinoDelete='PersonaEstudios_lista.php';
		...
		$sql->addField("pees_actualfecha", 'NOW()', "String");
		$sql->addField("pees_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    $otable = new TableSimple("","100%",16,'tLista'); 
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Grado Instrucci&oacute;n","18%", "C"); 
                        $otable->addColumnHeader("Centro de Estudio","25%", "C"); 
                        $otable->addColumnHeader("Situaci&oacute;n Academica","25%", "C"); 
                        $otable->addColumnHeader("Fec.Egresado","10%", "C");
                        $otable->addColumnHeader("Grado o T&iacute;tulo","30%", "C");
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("pees_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("peesid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaEstudios_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData($rs->field("grado_instruccion"));	
                            $otable->addData($rs->field("centro_estudio"));	
                            $otable->addData($rs->field("situacion_academica"));
                            $otable->addData(dtos($rs->field("pees_fegresado")));
                            $otable->addData($rs->field("grado_estudio"));                            
...
class clsPersonaEstudios_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(pees_id::TEXT,5,'0') AS peesid,  
                                      b.tabl_descripcion AS grado_instruccion, 
                                      c.cees_nombre AS centro_estudio, 
                                      d.siac_descripcion AS situacion_academica,
                                      e.gres_descripcion AS grado_estudio,  
                                      f.espe_descripcion AS especialidad,
                                      i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres AS empleado,
                                      i.pers_dni AS dni,
                                      x.usua_login AS username,
                                      y.usua_login AS usernameactual
			FROM personal.persona_estudios a 
			LEFT JOIN catalogos.tabla               b ON a.tabl_gradoinstruccion = b.tabl_id 
			LEFT JOIN personal.centro_estudio       c ON a.cees_id = c.cees_id
			LEFT JOIN personal.situacion_academica  d ON a.siac_id = d.siac_id
			LEFT JOIN personal.grado_estudio        e ON a.gres_id = e.gres_id  
                        LEFT JOIN personal.especialidades       f ON a.espe_id = f.espe_id  
                        LEFT JOIN personal.persona  i ON a.pers_id=i.pers_id
                        LEFT JOIN admin.usuario     x ON a.usua_id=x.usua_id
                        LEFT JOIN admin.usuario     y ON a.pees_actualusua=y.usua_id
	...
	function whereID($id){
		$this->addWhere("a.pees_id=$id");
	}
        
	function whereDescrip($descrip){
		$this->addWhere("(b.tabl_descripcion ILIKE '%$descrip%' 
                                    OR c.cees_nombre      ILIKE '%$descrip%' 
                                    OR d.siac_descripcion ILIKE '%$descrip%' 
                                    OR e.gres_descripcion ILIKE '%$descrip%' 
                                    OR f.espe_descripcion ILIKE '%$descrip%'     
                                 )");
	}

        function wherePeriodo($fechIni,$fechFin){
		$this->addWhere("(a.pees_fegresado>='$fechIni' AND a.pees_fegresado<='$fechFin')");
	}
        
        function whereGradoInstruccion($tabl_gradoinstruccion){
		$this->addWhere("a.tabl_gradoinstruccion=$tabl_gradoinstruccion");
	}
        
        function whereSituacionAcademica($siac_id){
		$this->addWhere("a.siac_id=$siac_id");
	}
        
	function orderUno(){
		$this->addOrder("a.pees_id DESC");
	}
        
        function orderDos(){
		$this->addOrder("i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres,a.pees_fegresado");
	}        
