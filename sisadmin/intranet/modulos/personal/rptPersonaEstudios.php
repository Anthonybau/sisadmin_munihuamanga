...
class Reporte extends GenReporte
{
	...
	function Cabecera(){
		...
		$this->Cell(24,$this->lineHeight,'GRADO','TLR',0,'C',1);	
                $this->Cell(50,$this->lineHeight,'CENTRO','TL',0,'C',1);
                $this->Cell(38,$this->lineHeight,'SITUACION','TL',0,'C',1);
                $this->Cell(18,$this->lineHeight,'FECHA','TL',0,'C',1);
                $this->Cell(50,$this->lineHeight,'GRADO O','TL',0,'C',1);
                $this->Cell(18,$this->lineHeight,'FECHA','TL',0,'C',1);
                $this->Cell(18,$this->lineHeight,utf8_decode('N°'),'TL',0,'C',1);
                $this->Cell(60,$this->lineHeight,'ESPECIALIDAD','TLR',0,'C',1);
                $this->Cell(15,$this->lineHeight,utf8_decode('N°'),'TLR',1,'C',1);

                $this->Cell(24,$this->lineHeight,'INSTRUCCION','BLR',0,'C',1);
		$this->Cell(50,$this->lineHeight,'ESTUDIOS','BLR',0,'C',1);
                $this->Cell(38,$this->lineHeight,'ACADEMICA','BLR',0,'C',1);
                $this->Cell(18,$this->lineHeight,'EGRES','BLR',0,'C',1);
                $this->Cell(50,$this->lineHeight,'TITULO','BLR',0,'C',1);
                $this->Cell(18,$this->lineHeight,'GRADO/TIT','BLR',0,'C',1);
                $this->Cell(18,$this->lineHeight,'TITULO','BLR',0,'C',1);
                $this->Cell(60,$this->lineHeight,'','BLR',0,'C',1);
                $this->Cell(15,$this->lineHeight,'COLEG.','BLR',0,'C',1);
	...
	function SeteoCampos(){
		...
		$this->addField('C1',  99999,	0,  24);
                $this->addField('C2',  99999,	0,  50);
                $this->addField('C3',  99999,	0,  38);
	...
	function Detalle(){
		global $rs;	
                $this->printField($rs->field("grado_instruccion"), 'C1','items',0,'L',true);
		$this->printField($rs->field("centro_estudio"), 'C2','items',0,'L',true);		
                $this->printField($rs->field("situacion_academica"), 'C3','items',0,'L',true);
                $this->printField(dtos($rs->field("pees_fegresado")), 'C4','items',0,'C');
                $this->printField($rs->field("grado_estudio"), 'C5','items',0,'L');                
                $this->printField(dtos($rs->field("pees_fgradotitulo")), 'C6','items',0,'C');
                $this->printField($rs->field("pees_ntitulo"), 'C7','items',0,'C');
                $this->printField($rs->field("especialidad"), 'C8','items',0,'L');
                $this->printField($rs->field("pees_ncolegiatura"), 'C9','items',0,'C');
...
/*	recibo los parametros */
	...
	$_titulo = "REGISTRO DE ESTUDIOS" ; // T?tulo del reporte
...
/*	establecer conexion con la BD */
	...
	$sql=new clsPersonaEstudios_SQLlista();
        ...
        if($tabl_gradoinstruccion){
            $sql->whereGradoInstruccion($tabl_gradoinstruccion);
        }
        
        if($siac_id){
            $sql->whereSituacionAcademica($siac_id);
        }
