...
class clsPersonaCuentaBancaria extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='personal.persona_cuenta_bancaria'; //nombre de la tabla
		$this->setKey='pecb_id'; //campo clave
		...
		$this->destinoInsert='PersonaCuentaBancaria_lista.php';
		$this->destinoUpdate='PersonaCuentaBancaria_buscar.php';
		$this->destinoDelete='PersonaCuentaBancaria_lista.php';
        ...
        function getSql(){
		$sql=new clsPersonaCuentaBancaria_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}

        function addField(&$sql){
		$sql->addField("pecb_actualfecha", 'NOW()', "String");
		$sql->addField("pecb_actualusua", getSession("sis_userid"), "String");
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
                ...
                if($busEmpty==1) { //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
                    ...
                    if ($rs->numrows()>0) {
                        ...
                        $otable->addColumnHeader("Tipo","5%", "C"); 
                        $otable->addColumnHeader("Fecha","10%", "C"); 
                        $otable->addColumnHeader("Banco","43%", "C"); 
                        $otable->addColumnHeader("Cuenta","40%", "C");
                        $otable->addRowHead();

                        while ($rs->getrow()) {
                            $id = $rs->field("pecb_id"); // captura la clave primaria del recordsource

                            $otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

                            $otable->addData(addLink($rs->field("pecbid"),PATH_INC."auxiliar.php?pag=../../modulos/personal/PersonaCuentaBancaria_edicion.php?id=$id,id_relacion=$id_relacion","Click aqu&iacute; para consultar o editar este registro","content","ls-modal"));	
                            $otable->addData($rs->field("tipo_cuenta"));	
                            $otable->addData(dtos($rs->field("pecb_fecha")));	
                            $otable->addData($rs->field("banco"));	
                            $otable->addData($rs->field("pecb_cuentadeposito"));
...
class clsPersonaCuentaBancaria_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT  a.*,
                                      LPAD(a.pecb_id::TEXT,5,'0') AS pecbid,  
                                      CASE WHEN a.pecb_tipo=1 THEN 'HABERES' ELSE 'CTS' END AS tipo_cuenta,
                                      b.tabl_descripcion AS banco,
                                      i.pers_apellpaterno||' '||i.pers_apellmaterno||' '||i.pers_nombres AS empleado,
                                      i.pers_dni AS dni,
                                      y.usua_login AS username,
                                      x.usua_login AS usernameactual
			FROM personal.persona_cuenta_bancaria a 
                        LEFT JOIN catalogos.tabla   b ON a.tabl_bancoid = b.tabl_id  
                        LEFT JOIN personal.persona  i ON a.pers_id=i.pers_id
                        LEFT JOIN admin.usuario y ON a.usua_id=y.usua_id
                        LEFT JOIN admin.usuario x ON a.pecb_actualusua=x.usua_id
                        ";
	}
	...
	function whereID($id){
		$this->addWhere("a.pecb_id=$id");
	}
        
	function whereDescrip($descrip){
		$this->addWhere("a. pecb_cuentadeposito ILIKE '%$descrip%'");
	}

	function orderUno(){
		$this->addOrder("a.pecb_id DESC");
	}
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsPersonaCuentaBancaria();
