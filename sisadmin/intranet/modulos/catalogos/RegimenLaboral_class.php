...
class clsRegimenLaboral extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='regimen_laboral'; //nombre de la tabla
		$this->setKey='rela_id'; //campo clave
		...
		$this->destinoUpdate = "RegimenLaboral_buscar.php";
		$this->destinoInsert = "RegimenLaboral_buscar.php";
		$this->destinoDelete = "RegimenLaboral_buscar.php";
                $this->pagEdicion ="RegimenLaboral_edicion.php";
                $this->pagBuscar="RegimenLaboral_buscar.php";
	}
	...
	function getSql(){
		$sql=new clsRegimenLaboral_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}
...

	{
		...
		if(strlen($cadena)>0 or $busEmpty==1){ //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
			...
			if ($rs->numrows()>0) {
					...
					while ($rs->getrow()) {
						$id = $rs->field("rela_id"); // captura la clave primaria del recordsource
						$campoTexto_de_Retorno = especialChar($rs->field("rela_descripcion"));
...
class clsRegimenLaboral_SQLlista extends selectSQL {

	function __construct(){
		$this->sql="SELECT  a.*,
					x.usua_login as username
				FROM catalogos.regimen_laboral a
	...
	function whereID($id){
		$this->addWhere("a.rela_id=$id");
	}

	function whereDescrip($descrip){
		if($descrip) $this->addWhere("a.rela_descripcion ILIKE '%$descrip%'");
	}

	
	function orderUno(){
		$this->addOrder("a.rela_id");
	}


	function getSQL_cbox(){
		$sql="SELECT rela_id,rela_descripcion
