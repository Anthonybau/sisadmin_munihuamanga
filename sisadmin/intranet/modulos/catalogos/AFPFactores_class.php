...
class clsAFPFactores extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='catalogos.afp_factores'; //nombre de la tabla
		$this->setKey='affa_id'; //campo clave
		...
		$this->destinoInsert='AFPFactores_buscar.php';
		$this->destinoUpdate='AFPFactores_buscar.php';
		$this->destinoDelete='AFPFactores_buscar.php';
        ...
        function getSql(){
		$sql=new clsAFPFactores_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}
		...
		$cadena=$cadena?$cadena:'';
			...
			if ($rs->numrows()>0) {
					...
					$otable->addColumnHeader("ID","5%", "L"); 
                                        $otable->addColumnHeader("Aplicable Desde","10%", "C"); 
					$otable->addColumnHeader("Comisi&oacute;n &Uacute;nica","15%","C");
                                        $otable->addColumnHeader("Comisi&oacute;n Mixta","15%","C");
                                        $otable->addColumnHeader("Prima","15%","C");
                                        $otable->addColumnHeader("Aporte","15%","C");
                                        $otable->addColumnHeader("Rem.Max.Aseg.","20%","C");
					...
					while ($rs->getrow()) {
						$id = $rs->field("id"); // captura la clave primaria del recordsource
                                                ...
                                                $otable->addData(dtos($rs->field("affa_fdesde")),"C");
						$otable->addData($rs->field("affa_comision"),"R");
                                                $otable->addData($rs->field("affa_comision_mixta"),"R");
                                                $otable->addData($rs->field("affa_prima"),"R");
                                                $otable->addData($rs->field("affa_aporte"),"R");
                                                $otable->addData($rs->field("affa_tope"),"R");
...
class clsAFPFactores_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT LPAD(a.affa_id::TEXT,4,'0') AS id,
                                    a.affa_fdesde,
                                    a.affa_comision,
                                    a.affa_comision_mixta,
                                    a.affa_prima,
                                    a.affa_aporte,
                                    a.affa_tope,
                                    b.afp_nombre,
                                    x.usua_login as username
			FROM catalogos.afp_factores a
                        LEFT JOIN catalogos.afp b ON a.afp_id=b.afp_id
                        LEFT JOIN usuario x ON a.usua_id=x.usua_id
                        ";
	}

	function wherePadreID($padre_id){
		$this->addWhere("a.afp_id=$padre_id");
	}

        
	function whereID($id){
		$this->addWhere("a.affa_id=$id");
	}
        
        
        function whereDescrip($descrip){
		$this->addWhere("TO_CHAR(a.affa_fdesde,'DD/MM/YYYY')::TEXT ILIKE '%$descrip%'");
	}

	function orderUno(){
		$this->addOrder("a.affa_id DESC");
	}
}
