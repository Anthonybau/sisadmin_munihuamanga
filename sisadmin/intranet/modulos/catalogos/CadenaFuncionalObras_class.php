...
class clsCadenaFuncionalObras extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='siscopp.componente_obras'; //nombre de la tabla
		$this->setKey='coob_id'; //campo clave
		...
		$this->destinoInsert='CadenaFuncionalObras_buscar.php';
		$this->destinoUpdate='CadenaFuncionalObras_buscar.php';
		$this->destinoDelete='CadenaFuncionalObras_buscar.php';
        ...
        function getSql(){
		$sql=new clsCadenaFuncionalObras_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='')
	{
		...
		$cadena=$cadena?$cadena:'';
			...
			if ($rs->numrows()>0) {
					...
					$otable->addColumnHeader("ID","10%", "L"); // T�tulo, Ordenar?, ancho, alineaci�n
					$otable->addColumnHeader("Proyecto/Obra","85%","L");
                                        $otable->addColumnHeader("Usuario","5%","L");
					$otable->addRowHead(); 					
					...
					while ($rs->getrow()) {
						...
						$otable->addData($rs->field("cfob_descripcion"));
...
class clsCadenaFuncionalObras_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT LPAD(a.coob_id::TEXT,4,'0') AS id,
                                    a.coob_descripcion,
                                    a.coob_habilitado,
                                    a.coob_generica,
                                    b.comp_mnemonico,
			...
			FROM siscopp.componente_obras a
                        LEFT JOIN siscopp.componente b ON a.comp_id=b.comp_id
                        LEFT JOIN admin.usuario x ON a.usua_id=x.usua_id
                        ";
	}

	function wherePadreID($padre_id){
		$this->addWhere("a.comp_id=$padre_id");
	}

        
	function whereID($id){
		$this->addWhere("a.coob_id=$id");
	}
        
        function wherePeriodo($anno){
		$this->addWhere("a.coob_anno=$anno");
	}
	...
	function whereDepeID($depe_id){
		...
		$this->addWhere("a.coob_descripcion ILIKE '%$descrip%'");
	}

	function orderUno(){
		$this->addOrder("a.coob_id DESC");
	}
        
        function getSQL_cbox(){
                        $sql="SELECT a.id,a.id||' '||a.comp_mnemonico||'-'||a.peri_anno::TEXT||' '||a.coob_descripcion 
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsCadenaFuncionalObras();
