...
class clsCategoriaRemunerativa extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='categoria_remunerativa'; //nombre de la tabla
		$this->setKey='care_id'; //campo clave
		...
		$this->destinoUpdate = "CategoriaRemunerativa_buscar.php";
		$this->destinoInsert = "CategoriaRemunerativa_buscar.php";
		$this->destinoDelete = "CategoriaRemunerativa_buscar.php";
                $this->pagEdicion ="CategoriaRemunerativa_edicion.php";
                $this->pagBuscar="CategoriaRemunerativa_buscar.php";
	}
	...
	function getSql(){
		$sql=new clsCategoriaRemunerativa_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}
...

	{
		...
		if(strlen($cadena)>0 or $busEmpty==1){ //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias
			...
			if ($rs->numrows()>0) {
					...
					$paramFunction->replaceParValue('colOrden',$colOrden++);
					$otable->addColumnHeader("Descripci&oacute;n",true,"75%", "L","javascript:xajax_buscar(1,xajax.getFormValues('frm'),'".encodeArray($paramFunction->getUrl())."','$pg','$Nameobj')"); 
					$paramFunction->replaceParValue('colOrden',$colOrden++);
					$otable->addColumnHeader("Condici&oacute;n",true,"10%", "L","javascript:xajax_buscar(1,xajax.getFormValues('frm'),'".encodeArray($paramFunction->getUrl())."','$pg','$Nameobj')"); 
					$paramFunction->replaceParValue('colOrden',$colOrden++);
					$otable->addColumnHeader("Jornal Diario",true,"10%", "L","javascript:xajax_buscar(1,xajax.getFormValues('frm'),'".encodeArray($paramFunction->getUrl())."','$pg','$Nameobj')"); 
                                        $paramFunction->replaceParValue('colOrden',$colOrden++);
					$otable->addColumnHeader("Usuario",true,"5%", "L","javascript:xajax_buscar(1,xajax.getFormValues('frm'),'".encodeArray($paramFunction->getUrl())."','$pg','$Nameobj')");                                         
					$otable->addRow(); // adiciona la linea (TR)
					while ($rs->getrow()) {
						$id = $rs->field("care_id"); // captura la clave primaria del recordsource
						$campoTexto_de_Retorno = especialChar($rs->field("care_descripcion"));
						...
						$otable->addData($rs->field("care_descripcion"));
                                                $otable->addData($rs->field("sit_laboral"));
                                                $otable->addData(number_format($rs->field("care_jornal"), 2, '.', ','),"R");
...
class clsCategoriaRemunerativa_SQLlista extends selectSQL {

	function __construct(){
		$this->sql="SELECT  a.*,
                                    b.tabl_descripcorta AS sit_laboral,
					x.usua_login as username
				FROM catalogos.categoria_remunerativa a
                                LEFT JOIN catalogos.tabla b ON a.tabl_idsitlaboral=b.tabl_id
	...
	function whereID($id){
		$this->addWhere("a.care_id=$id");
	}

	function whereSitLaboral($tabl_idsitlaboral){
		$this->addWhere("a.tabl_idsitlaboral=$tabl_idsitlaboral");
	}

        
	function whereDescrip($descrip){
		if($descrip) $this->addWhere("a.care_descripcion ILIKE '%$descrip%'");
	}

	
	function orderUno(){
		$this->addOrder("a.care_id DESC");
	}


	function getSQL_cbox(){
		$sql="SELECT care_id,care_descripcion
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsCategoriaRemunerativa();
