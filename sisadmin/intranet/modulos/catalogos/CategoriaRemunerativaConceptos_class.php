...
class clsCategoriaRemunerativaConceptos extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='categoria_remunerativa_conceptos'; //nombre de la tabla
		$this->setKey='crco_id'; //campo clave
		...
		$this->destinoInsert='CategoriaRemunerativaConceptos_buscar.php';
		$this->destinoUpdate='CategoriaRemunerativaConceptos_buscar.php';
		$this->destinoDelete='CategoriaRemunerativaConceptos_buscar.php';
        ...
        function getSql(){
		$sql=new clsCategoriaRemunerativaConceptos_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}
        ...
        function buscar($op,$formData,$arrayParam,$pg,$Nameobj='')
	{
		...
		$cadena=$cadena?$cadena:'';
			...
			if ($rs->numrows()>0) {
					...
					$otable->addColumnHeader("Descripci&oacute;n","40%","L");
                                        $otable->addColumnHeader("Desde","5%","L");
                                        $otable->addColumnHeader("Hasta","5%","L");
                                        $otable->addColumnHeader("Categoria","20%","L");
                                        $otable->addColumnHeader("Clasif","5%","L");
                                        $otable->addColumnHeader("Porcentaje","5%","C");
                                        $otable->addColumnHeader("Importe","10%","R");
                                        //$otable->addColumnHeader("Formula","20%","C");
                                        $otable->addColumnHeader("Usuario","5","R");
					$otable->addRowHead(); 					
					...
					while ($rs->getrow()) {
						$id = $rs->field("id"); // captura la clave primaria del recordsource
                                                ...
                                                $otable->addData($rs->field("servid"));
						$otable->addData($rs->field("serv_descripcion"));
                                                $otable->addData($rs->field("mes_desde"));
                                                $otable->addData($rs->field("mes_hasta"));
                                                $otable->addData($rs->field("categoria_concepto"));
                                                $otable->addData($rs->field("clas_id"));
                                                if($rs->field("crco_porcentaje")>0){
                                                    $otable->addData($rs->field("crco_porcentaje"),"C");
                                                }else{
                                                    $otable->addData("&nbsp;");
                                                }
                                                    
                                                if($rs->field("crco_importe")>0){
                                                    $otable->addData($rs->field("crco_importe"),"R");
                                                }else{
                                                    $otable->addData("&nbsp;");
                                                }
                                                //$otable->addData($rs->field("conc_formula"));
	...
	function buscarAgregar($op,$formData,$arrayParam,$pg,$Nameobj='')
	{
		global $conn,$param;
		$objResponse = new xajaxResponse();
		...
		if(strlen($cadena)>0 || $busEmpty==1){ //el proceso solo se ejecuta si la cadena contiene valor o se permite cadenas vacias

			$sql=new clsConceptos_x_Agregar_SQLlista(); //JORNALES
                        $sql->whereNOAgregados($relacionamiento_id);
                        $sql->whereTiplID(8);
                        $sql->whereNoAutomatico();
...
//			$objResponse->addAlert($sql);
			...
			if ($rs->numrows()>0) {
					...
					$paramFunction->replaceParValue('colOrden',$colOrden++);
					$otable->addColumnHeader("Descripci&oacute;n",true,"70%", "L","javascript:xajax_buscar(1,xajax.getFormValues('frm'),'".encodeArray($paramFunction->getUrl())."','$pg','$Nameobj')"); 
                                        $paramFunction->replaceParValue('colOrden',$colOrden++);
					$otable->addColumnHeader("Categor&iacute;a",true,"25%", "L","javascript:xajax_buscar(1,xajax.getFormValues('frm'),'".encodeArray($paramFunction->getUrl())."','$pg','$Nameobj')");                                         
					...
					while ($rs->getrow()) {
						...
						//$otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" onclick=\"checkform(frm,this)\">");
                                                $otable->addData("<input type=\"button\" class=\"botonAgg\" value=\"Elegir\" id=\"adi_$ndiv\" onClick=\"xajax_elegir('$relacionamiento_id','$id','".$rs->numrows()."','addDatos_$ndiv');\">");
                                                $otable->addData($rs->field("id"));
                                                $otable->addData($rs->field("serv_descripcion"));
                                                $otable->addData($rs->field("categoria_concepto"));
						$otable->addRow();
                                                
                                                $otable->addHtml("<div><tr><td colspan=11><div id=\"addDatos_$ndiv\">");
                                                $otable->addHtml("</div></td></tr></div>\n");				
...
class clsCategoriaRemunerativaConceptos_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "  SELECT a.crco_id AS id,
                                    a.serv_codigo,
                                    a.clas_id,
                                    LPAD(a.serv_codigo::TEXT,5,'0') AS servid,
                                    a.crco_porcentaje,
                                    a.crco_importe,
                                    c.serv_descripcion,
                                    c.serv_breve,
                                    c.serv_formula,
                                    b.care_descripcion,
                                    d.tabl_descripcion AS mes_desde,
                                    e.tabl_descripcion AS mes_hasta,
                                    f.sesg_descripcion AS categoria_concepto,
                                    x.usua_login AS username 
			FROM catalogos.categoria_remunerativa_conceptos a
                        LEFT JOIN catalogos.categoria_remunerativa b ON a.care_id=b.care_id 
                        LEFT JOIN catalogos.servicio c ON a.serv_codigo=c.serv_codigo
                        LEFT JOIN catalogos.tabla d ON a.tabl_mesini=d.tabl_id 
                        LEFT JOIN catalogos.tabla e ON a.tabl_mesfin=e.tabl_id 
                        LEFT JOIN catalogos.servicio_sgrupo f ON c.sesg_id=f.sesg_id
                        LEFT JOIN admin.usuario x ON a.usua_id=x.usua_id
                        ";
	}

	function wherePadreID($care_id){
		$this->addWhere("a.care_id=$care_id");
	}

        
	function whereID($id){
		$this->addWhere("a.crco_id=$id");
	}
        
        
        function whereDescrip($descrip){
		$this->addWhere("c.serv_descripcion ILIKE '%$descrip%'");
	}

	function orderUno(){
		$this->addOrder("a.crco_id DESC");
	}
        
	function orderDos(){
		$this->addOrder("c.sesg_id,a.serv_codigo DESC");
	}
}


class clsConceptos_x_Agregar_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT LPAD(a.serv_codigo::TEXT,5,'0') AS id,
                                     a.*,
                                     d.sesg_descripcion AS categoria_concepto 
                            FROM catalogos.servicio a
                            LEFT JOIN catalogos.servicio_sgrupo d ON a.sesg_id=d.sesg_id
	...
	function whereID($id){
        ...
        function whereNOAgregados($care_id){
                 $this->addWhere("a.serv_codigo NOT IN (SELECT serv_codigo FROM catalogos.categoria_remunerativa_conceptos WHERE care_id=$care_id)");
	}                   
                                    
        function whereTiplID($tipl_id){
                $this->addWhere("COALESCE(a.tipl_id,0)=$tipl_id");
        }
	function whereDescrip($descrip){
		$this->addWhere("a.serv_descripcion ILIKE '%$descrip%'");
	}

        function whereNoAutomatico(){
		$this->addWhere("a.serv_automatico=0");
	}        
        
	function orderUno(){
		$this->addOrder("a.sesg_id,a.serv_codigo");
	}
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsCategoriaRemunerativaConceptos();
