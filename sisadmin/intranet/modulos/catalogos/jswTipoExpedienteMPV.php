...
if($depe_id>=0){   
    ...
    try {
        ...
        $sqlTipoExpediente=new clsTipExp_SQLlista();
        $sqlTipoExpediente->whereMPVirtual();
        $sqlTipoExpediente->whereHabilitado();                

        $data = $db->query($sqlTipoExpediente->getSQL_cbox2())->fetchAll( PDO::FETCH_ASSOC);
