...
class clsValorUIT extends entidad {
	
	function __construct($id='',$title=''){
		$this->setTable='valor_uit'; //nombre de la tabla
		$this->setKey='vaui_id'; //campo clave
...
//		$this->pagBuscar  = "catalogosDependenciaAdquiriente_buscar.php";		
	...
	function eliminar(){
		...
		if($error) alert($error);
			...
			//*TIP CON POSTGRES, SE NECESITA DOBLE ELIMINACION*/
			$conn->execute($sql);
			$error=$conn->error();		
			if($error) alert($error);
			else		
...
//						$otable->addColumnHeader("<input type=\"checkbox\" id=\"checkall\" >"); // Coluna com checkbox
					...
					$otable->addColumnHeader("A&ntilde;o","10%", "L"); 
					$otable->addColumnHeader("Valor","85%", "L"); 
                                        $otable->addColumnHeader("Usuario","5%", "L"); 
					$otable->addRowHead(); 					

					while ($rs->getrow()) {
						$id = $rs->field("vaui_id"); // captura la clave primaria del recordsource
						
						$otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" >");

						$otable->addData(addLink($rs->field("vaui_anno"),$pageEdit."?id=$id&Sbusc_tido_id=$tido_id&".$param->buildPars(false),"Click aqu&iacute; para consultar o editar el registro"));
                                                        
						$otable->addData($rs->field("vaui_valor"));
...
class clsValorUIT_SQLlista extends selectSQL {
	...
	function whereID($id){
		$this->addWhere("a.vaui_id=$id");	
	}

	function whereAnno($anno){
		$this->addWhere("a.vaui_anno=$anno");	
	}
	
	
	function orderUno(){
		$this->addOrder("a.vaui_anno DESC");		
	}
