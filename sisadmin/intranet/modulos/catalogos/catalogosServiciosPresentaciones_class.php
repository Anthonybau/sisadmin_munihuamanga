...
class clsServicioPresentaciones extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='catalogos.servicio_precios'; //nombre de la tabla
		$this->setKey='sepr_id'; //campo clave
		...
		$this->destinoInsert='catalogosServiciosPresentaciones_lista.php';
		$this->destinoUpdate='catalogosServiciosPresentaciones_lista.php';
		$this->destinoDelete='catalogosServiciosPresentaciones_lista.php';
        ...
        function getSql(){
		$sql=new clsServicioPresentaciones_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}

        function addField(&$sql){
		$sql->addField("sepr_actualfecha", 'NOW()', "String");
		$sql->addField("sepr_actualusua", getSession("sis_userid"), "String");                
	}
	...
	function activar(){
		...
		/* Sql a ejecutar */
		$sqlCommand="UPDATE $this->setTable SET sepr_estado=CASE WHEN sepr_estado=1 THEN 9 ELSE 1 END ";
...
class clsServicioPresentaciones_SQLlista extends selectSQL {
	function __construct(){
		$this->sql = "SELECT a.*,
                                    c.tabl_descripcion AS umedida,
                                    d.tabl_descripcion AS tipo_precio,
                                    ...
                                    LEFT JOIN catalogos.tabla c                 ON a.tabl_umedida=c.tabl_id
                                    LEFT JOIN catalogos.tabla d                 ON a.tabl_tipoprecio=d.tabl_id
                                    LEFT JOIN admin.usuario x                   ON a.usua_id=x.usua_id 
                                    LEFT JOIN admin.usuario y                   ON a.sepr_actualusua=y.usua_id 
                            ";
	}
	...
	function wherePadreID($padre_id){
		...
		$this->addWhere('a.sepr_estado=1');	
	}
        
        function whereNOActivo(){
		$this->addWhere("a.sepr_estado=9");	
	}        

	function whereID($id){
		$this->addWhere("a.sepr_id=$id");
	}
        
	function orderUno(){
		$this->addOrder("a.sepr_id DESC");
	}        
}
...
if (isset($_GET["control"])){
    ...
    if($control){
            ...
            $dml=new clsServicioPresentaciones();
