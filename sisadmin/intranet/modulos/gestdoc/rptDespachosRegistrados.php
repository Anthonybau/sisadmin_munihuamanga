...
class Reporte extends GenReporte
{
	...
	function Cabecera(){
		...
		$this->Cell(40,'','TIPO DE '.NAME_EXPEDIENTE_UPPER.': '.$tipoDespacho,0,0,'L',1);
	...
	function TituloGrupo1(){
		global $rs;
		$this->beginBlock();
		$this->printField("DEPENDENCIA QUE REGISTRA: ".$rs->field("depe_nombre_genera"). ' / '.$rs->field("depe_superior_nombre_genera"), 'HG1','bold','T','L');
	}
	...
	function TituloGrupo2(){
		...
		$this->beginBlock();
		$this->printField('        USUARIO QUE REGISTRA: '.$rs->field("usuario_genera")." [".$usuario_nombre_completo."]", 'HG1','bold','B','L');
	}
	...
	function Detalle(){
		global $rs,$ult_desp_id;
                if($ult_desp_id!=$rs->field("id_padre")){
                    ...
                    $this->printField(date("d/m/Y",strtotime($rs->field('desp_fregistro'))),  'C2','items','','L');
