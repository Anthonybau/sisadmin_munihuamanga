...
class Reporte extends GenReporte
{
	function SeteoPdf(){
		...
		$this->CampoGrupo2='usua_id'; // Voy a tener el Grupo 1 agrupado por el campo repe_descripcion
	...
	function TituloGrupo1(){
		global $rs;
		$this->beginBlock();
		$this->printField("DEPENDENCIA ORIGEN: ".$rs->field("depe_nombre_genera").' / '.$rs->field("depe_superior_nombre_genera"), 'HG1','bold','T','L');
	}
	...
	function TituloGrupo2(){
		global $rs;
                $usuario_nombre_completo=GetDbValue("SELECT pers_nombres||' '||pers_apelLpaterno||' '||pers_apeLlmaterno
                                                     ...
                                                     (SELECT pdla_id FROM usuario WHERE usua_id=".$rs->field('usua_id')."))");
		$this->beginBlock();
		$this->printField('        USUARIO ORIGEN: '.$rs->field("usuario_genera")." [".utf8_decode($usuario_nombre_completo)."]", 'HG1','bold','B','L');
	}


	function Detalle(){
		global $rs,$ult_desp_id;
                if($ult_desp_id!=$rs->field("id_padre")){
                    ...
                    $this->printField($rs->field("depe_nombrecorto_destino"),  'C6','items','','L');
	...
	function PieGrupo2(){
		...
		$this->beginBlock();
		$this->printField('        Total Derivaciones en Usuario:', 'HG2','bold',0,'L');
		$this->printField(number_format($this->functions['CONT_GRUPO2']['C6'], 0, '.', ','), 'HG3','bold',0,'R');
	}

	function PieGrupo1(){
		$this->Line($this->blockPosX, $this->blockPosY+$this->lasth,$this->blockPosX+$this->WidthTotalCampos, $this->blockPosY+$this->lasth); // Imprimo Línea al final de cada grupo
		$this->beginBlock();
		$this->printField('Total Derivaciones en Dependencia:', 'HG2','bold',0,'L');
		$this->printField(number_format($this->functions['CONT_GRUPO1']['C6'], 0, '.', ','), 'HG3','bold',0,'R');
	}
	...
	function Summary(){
		...
		$this->printField(number_format($this->functions['CONT_TOTAL']['C6'], 0, '.', ','), 'HG3','bold',0,'R');
...
$nrbusc_depe_destinoid  = getParam("nbusc_depe_destinoid");
$nrbusc_user_id_destino  = getParam("nbusc_user_id_destino");
...
if($nbusc_user_id){
    $sql->whereUsuaOrigen($nbusc_user_id);
}
...
if($nrbusc_depe_destinoid){
    $sql->whereDepeDestino($nrbusc_depe_destinoid);
}

if($nrbusc_user_id_destino){
    $sql->whereUsuaDestino($nrbusc_user_id_destino);
}
