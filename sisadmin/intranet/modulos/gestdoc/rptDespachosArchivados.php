...
class Reporte extends GenReporte
{
	function SeteoPdf(){
		...
		$this->CampoGrupo2='arch_id'; // Voy a tener el Grupo 1 agrupado por el campo repe_descripcion
                $this->CampoGrupo3='usua_idarchiva'; // Voy a tener el Grupo 1 agrupado por el campo repe_descripcion
	...
	function Cabecera(){
		$this->Cell(15,$this->lineHeight+1,NAME_EXPEDIENTE_UPPER,1,0,'C',1);
                $this->Cell(24,$this->lineHeight+1,'ARCHIVADO',1,0,'C',1);
	...
	function SeteoCampos(){
		...
		$this->addField('HG2',  0,	0,	42);
		$this->addField('HG3',  42,	0,	20);
	...
	function TituloGrupo1(){
		global $rs;
		$this->beginBlock();
		$this->printField("DEPENDENCIA QUE ARCHIVA: ".$rs->field("depe_nombre_destino").' / '.$rs->field("depe_superior_nombre_destino"), 'HG1','bold','T','L');
	}

	function TituloGrupo2(){
		global $rs;
		$this->beginBlock();
		$this->printField("   ARCHIVADOR: ".$rs->field("archivador")." [".$rs->field("tipo_archivador")."]", 'HG1','bold','T','L');
	}
	...
	function TituloGrupo3(){
		global $rs;
                $usuario_nombre_completo=GetDbValue("SELECT pers_nombres||' '||pers_apelLpaterno||' '||pers_apeLlmaterno
                                                     ...
                                                     (SELECT pdla_id FROM usuario WHERE usua_id=".$rs->field('usua_idarchiva')."))");
		$this->beginBlock();
		$this->printField('        USUARIO QUE ARCHIVA: '.$rs->field("usuario_archiva")." [".$usuario_nombre_completo."]", 'HG1','bold','B','L');
	}
	...
	function Detalle(){
		...
		$this->printField($rs->field("id_padre"),  'C1','items','','L');
                $this->printField(date("d/m/Y i:m:s",strtotime($rs->field('dede_fechaarchiva'))),  'C2','items','','L');
		$this->printField($rs->field("tiex_abreviado").' '.$rs->field("num_documento"),  'C3','items','','L');
                ...
                $this->printField($rs->field("depe_nombrecorto_origen"). '/'.$rs->field("depe_superior_nombre_origen"),  'C6','items','','L');
	...
	function PieGrupo2(){
		$this->Line($this->blockPosX, $this->blockPosY+$this->lasth,$this->blockPosX+$this->WidthTotalCampos, $this->blockPosY+$this->lasth); // Imprimo Línea al final de cada grupo
		$this->beginBlock();
		$this->printField('        Total Registros en Archivador:', 'HG2','bold',0,'L');
...
$nbusc_arch_id  = getParam("nbusc_arch_id");
...
if($nbusc_user_id)
    $sql->whereUsuaArchiva($nbusc_user_id);

if($nbusc_arch_id)
    $sql->whereArchivador($nbusc_arch_id);
...
if($nrbusc_fdesde)
    $sql->whereFechaArchivaDesde($nrbusc_fdesde);

if($nrbusc_fhasta)
    $sql->whereFechaArchivaHasta($nrbusc_fhasta);
