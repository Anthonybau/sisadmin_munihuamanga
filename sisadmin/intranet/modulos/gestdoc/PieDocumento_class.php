...
class PieDocumento extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='gestdoc.pie_documento'; //nombre de la tabla
		$this->setKey='pido_id'; //campo clave
		...
		$this->title=$title;	
                ...
                $this->destinoUpdate = 'PieDocumento_edicion.php';
                $this->destinoInsert = 'PieDocumento_edicion.php';
	...
	function getSql(){
		$sql=new PieDocumento_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}

	function addField(&$sql){
		$sql->addField("pido_actualfecha", date('d/m/Y').' '.date('H:i:s'), "String");
		$sql->addField("pido_actualusua", getSession("sis_userid"), "String");
                
                if ($_POST["hx_pido_estado"]){
                    $sql->addField("pido_estado", 1, "Number");
                }else{
                    $sql->addField("pido_estado", 0, "Number");
                }
    ...
    function buscar($op,$formData,$arrayParam,$pg,$Nameobj='') {
		...
		/* Agrego las cabeceras de mi tabla */
                ...
                while ($rs->getrow()) {
                    $id = $rs->field("pido_id"); // captura la clave primaria del recordsource
                    ...
                    //if($usua_id==getSession("sis_userid")){ /* Si AUN NO SE RECIBE y ES EL USUARIO*/
                        $otable->addData(addLink($rs->field("id"),"PieDocumento_edicion.php?id=$id&clear=1&".$param->buildPars(false),"Click aqu&iacute; para consultar o editar el registro"),"C");
                    //}else{
                    //    $otable->addData($numero,"C");
                    //}
                    $otable->addData($rs->field("pido_nombre"));
                    $otable->addData($rs->field("username"),"C");

                    if($rs->field("pido_estado")==9){
	...
	function activar(){
		...
		/* Sql a ejecutar */
		$sqlCommand="UPDATE $this->setTable SET pido_estado=CASE WHEN pido_estado=1 THEN 9 ELSE 1 END ";
...
class PieDocumento_SQLlista extends selectSQL {

	function __construct(){
		$this->sql="SELECT  a.*,
                                    LPAD(a.pido_id::TEXT,4,'0') AS id,
                                    CASE WHEN a.pido_estado=1 THEN 'ACTIVO'
                                         WHEN a.pido_estado=9 THEN 'ANULADO'
                                    END AS estado,
                                    x.usua_login as username,
                                    y.usua_login as usernameactual
				FROM gestdoc.pie_documento a
				LEFT JOIN admin.usuario x ON  a.usua_id=x.usua_id
                                LEFT JOIN admin.usuario y ON  a.pido_actualusua=y.usua_id
				";
	
	}
	
	function whereID($id){
		$this->addWhere("a.pido_id=$id");
	}
       
	function whereDescrip($contenido){
		$this->addWhere("(a.pido_titulo ILIKE '%$contenido%')");
	}

        function whereActivo(){
		$this->addWhere("a.pido_estado=1");
	}

	function orderUno(){
            $this->addOrder("a.pido_id DESC");
	}
        
	function getSQL_cbox(){
		$sql="SELECT    a.pido_id,
                                a.id||' '||a.pido_nombre
