...
class tamanoPagina extends entidad {

	function __construct($id='',$title=''){
		$this->setTable='gestdoc.tamano_pagina'; //nombre de la tabla
		$this->setKey='tapa_id'; //campo clave
		...
		$this->destinoUpdate = "tamanoPagina_buscar.php";
		$this->destinoInsert = "tamanoPagina_buscar.php";
		$this->destinoDelete = "tamanoPagina_buscar.php";
	}

	function addField(&$sql){
		$sql->addField("tapa_actualfecha", "now()", "String");
		$sql->addField("tapa_actualusua", getSession("sis_userid"), "String");

	}

	function getSql(){
		$sql=new tamanoPagina_SQLlista();
		$sql->whereID($this->id);
		return($sql->getSQL());
	}
...
//			$objResponse->addAlert($sql);
			...
			if ($rs->numrows()>0) {
					...
					$otable->addColumnHeader("Nombre",false,"95%", "L");
					$otable->addRow(); // adiciona la linea (TR)
					while ($rs->getrow()) {
						$id = $rs->field("tapa_id"); // captura la clave primaria del recordsource
						$otable->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\" onclick=\"checkform(frm,this)\">");
						$otable->addData(addLink(str_pad($id,4,'0',STR_PAD_LEFT),"tamanoPagina_edicion.php?id=$id&".$param->buildPars(false),"Click aqu&iacute; para consultar o editar el registro"));
						
						$otable->addData($rs->field("tapa_nombre"));
						$otable->addRow();
					}
	...
	function guardar(){
		...
		$sql->setKey($this->setKey, $this->valueKey, $this->typeKey);
                ...
                $location=$_SERVER['DOCUMENT_ROOT'] ."/docs/gestdoc/margenes/".SIS_EMPRESA_RUC;
...
class tamanoPagina_SQLlista extends selectSQL {

	function __construct(){
		$this->sql="SELECT  a.*,
                                    x.usua_login as username,
                                    y.usua_login as usernameactual
				FROM gestdoc.tamano_pagina a
				LEFT JOIN admin.usuario x ON a.usua_id=x.usua_id
				LEFT JOIN admin.usuario y ON a.tapa_actualusua=y.usua_id
				";
	
	}
	
	function whereID($id){
		$this->addWhere("a.tapa_id=$id");
	}

        
	function whereDescrip($descrip){
		if($descrip) $this->addWhere("a.tapa_nombre ILIKE '%$descrip%'");
	}

        
	function orderUno(){
		$this->addOrder("a.tapa_id DESC");
	}


        
	function getSQL_cbox(){
		$sql="SELECT    tapa_id AS id,
                                LPAD(tapa_id::TEXT,4,'0')||' '||a.tapa_nombre AS descripcion
