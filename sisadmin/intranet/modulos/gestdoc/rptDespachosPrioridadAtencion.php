...
class Reporte extends GenReporte
{
	...
	function Cabecera(){
                global $prioridad_atencion_nombre;
                // Aquí­ imprimo los campos como titulos para el cuerpo del discurso
		$this->Cell(40,'','PRIORIDAD DE ATENCION: '.$prioridad_atencion_nombre,0,0,'L',1);
                $this->MultiCell(8,$this->lineHeight-1,'',0,0,'C',1);
		$this->SetX($this->blockPosX);

		$this->Cell(14,$this->lineHeight+1,NAME_EXPEDIENTE_UPPER,1,0,'C',1);
                $this->Cell(14,$this->lineHeight+1,'RECIBIDO',1,0,'C',1);
                $this->Cell(17,$this->lineHeight+1,'DIAS EXCED',1,0,'C',1);
	...
	function SeteoCampos(){
		...
		$this->addField('C1', 	99999,    0,	14);
                $this->addField('C2', 	99999,    0,	14);
		$this->addField('C5', 	99999,    0,	17);
	...
	function TituloGrupo1(){
		global $rs;
		$this->beginBlock();
		$this->printField("UBICACION: ".$rs->field("depe_nombre_destino").' / '.$rs->field("depe_superior_nombre_destino"), 'HG1','bold','T','L');
	}
	...
	function Detalle(){
		global $rs,$nrbusc_fproceso,$prioridad_atencion_dias;

		$this->printField($rs->field("id_padre"),  'C1','items','','L');

                $fecha_recibe=date("d/m/Y",strtotime($rs->field('dede_fecharecibe')));

                $this->printField($fecha_recibe,  'C2','items','','L');
                $this->printField(saca_dias_excedidos($nrbusc_fproceso,$fecha_recibe,$prioridad_atencion_dias),'C5','items','','C');
...
$nbusc_prat_id  = getParam("nbusc_prat_id");
...
$nrbusc_NTdesde  = getParam("nrbusc_NTdesde");
$nrbusc_NThasta  = getParam("nrbusc_NThasta");
$nrbusc_fproceso = getParam("nrbusc_fproceso");
...
$sql->wherePrioridadAtencion($nbusc_prat_id);
$sql->whereEnProceso();
...
if($nrbusc_NTdesde)
    $sql->whereDespachoDesde($nrbusc_NTdesde);

if($nrbusc_NThasta)
    $sql->whereDespachoHasta($nrbusc_NThasta);
...
$Subtitle.="NUMERO DE ".NAME_EXPEDIENTE_UPPER."  DESDE: ".str_pad($nrbusc_NTdesde,8,'0',STR_PAD_LEFT)." HASTA: ".str_pad($nrbusc_NThasta,8,'0',STR_PAD_LEFT);
$pdf->setTitle($_titulo." PROCESADO AL: ".$nrbusc_fproceso);
$pdf->setSubTitle($Subtitle);

$periodoAtencion=new clsPrioriAtencion_SQLlista();
$periodoAtencion->whereID($nbusc_prat_id);
$rsPA = new query($conn, $periodoAtencion->getSQL());
$rsPA->getrow();
$prioridad_atencion_nombre=$rsPA->field("prat_descripcion_larga");
$prioridad_atencion_dias=$rsPA->field("prat_dias");
