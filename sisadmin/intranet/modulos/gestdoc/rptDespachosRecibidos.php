...
class Reporte extends GenReporte
{
	...
	function TituloGrupo1(){
		global $rs;
		$this->beginBlock();
		$this->printField("DEPENDENCIA QUE RECIBE: ".$rs->field("depe_nombre_destino").' / '.$rs->field("depe_superior_nombre_destino"), 'HG1','bold','T','L');
	}
	...
	function TituloGrupo2(){
		...
		$this->beginBlock();
		$this->printField('        USUARIO QUE RECIBE: '.$rs->field("usuario_recibe")." [".$usuario_nombre_completo."]", 'HG1','bold','B','L');
	}
...
$nbusc_depe_procedeid=getParam("nbusc_depe_procedeid");
...
if($nbusc_depe_procedeid)
    $sql->whereDepeOrigen($nbusc_depe_procedeid);
