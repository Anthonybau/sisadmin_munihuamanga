...
//				   nErrTot+=1;}                         
			 ...
			 if (sNom.substring(0,1).toUpperCase()=='D' && objform.elements[j].value!=''){  
                                 var reg = /(19|20)\d\d[- /.](0[1-9]|1[012])[- /.](0[1-9]|[12][0-9]|3[01])/;
				 if (!objform.elements[j].value.match(reg)){
                                   if(!isDate(objform.elements[j].value)){
                                        sError+="Valor de campo '"+objform.elements[j].id+"' no es valido (formato de fecha debe ser : dd/mm/yyyy)"+"\r" ;
                                        nErrTot+=1;
                                    } 
                                 } 
...
//Non-Matches:  [24:00], [23:60]  
	...
	if(iKeyCode==13 && sOne.toUpperCase()!='E' )
		{
			...
			if(sOne.toUpperCase()=='N'  || sOne.toUpperCase()=='Z' || sOne.toUpperCase()=='R'){
				/*
				var valpad=''			
	...
	if ((sOne.toUpperCase()=='N' || sOne.toUpperCase()=='Z' || sOne.toUpperCase()=='R') && (pDecimal==undefined || pDecimal==0)){
		if((iKeyCode<48 || iKeyCode>59) && iKeyCode!=45 && iKeyCode!=8 && iKeyCode!=0)
			{return false;}
	}

	//DECIMAL
	if ((sOne.toUpperCase()=='N' || sOne.toUpperCase()=='R') && pDecimal>0){
...
El script se encarga de colocar los separadores pertinentes en el lugar indicado determinado por una lista en un campo de texto mientras se est� tecleando.
Sirve para m�s de un campo con distintos formatos de m�scaras.
Configuraci�n
...
Si lo quisi�ramos aaaa/mm/dd:
var patron = new Array(4,2,2)
...o un n�mero de tel�fono tipo 34-206-21-22:
var patron = new Array(2,3,2,2)
Si queremos usar el mismo script para distintos campos con distintas m�scaras habermos de crear un array para cada uno
var patron2 = new Array(1,9)
Finalmente habremos de llamar al script desde el/los campo/s pas�ndole como par�metros a s� mismo, el patr�n (array) a utilizar,
el separador que queramos aplicar y si queremos que s�lo acepte n�meros o no.
mascara(this,array a utilizar, separador, true si s�lo n�meros o false si cualquier caracter)
...
/* FUNCION QUE MUESTRA UN MENSAJE O UNA ANIMACION CUANDO SE EFECTUA UN PROCESO  
	...
	.........
	.........
	wait('')  Para eliminar la animaci�n, luego que termina el proceso 
*/
...
function wait(txtwait) {
	...
	cmd = "top.frames[a].document.getElementById('procesando').innerHTML = '<img src=\"../img/"+txtwait+"\"/>'"
	eval(cmd)
}

/* Funci�n que se usa para validar extensi�n de un archivo a subir */
function validaextension(idObj,cExtenciones){
	...
	if(nPosiExt<0){	
		idObj.value = ''		
		alert('Se aceptan archivos solo del tipo: '+cExtenciones+'. por favor corrija!')
	}
}
...
function AbreVentana(sURL) {
	...
	venrepo=window.open(sURL+char+'q=cat&' + oDate.getTime(),'rptSalida', "status=yes,resizable=yes,toolbar=no,scrollbars=yes,top=0,left=0,width=" + w + ",height=" + h, 1 );
	venrepo.focus();
}
...
/*Funcion que permite ver cualquier Archivo*/
function lectorPDF(file,title){        
            var oDate = new Date();
            var element = document.getElementById('myIframelectorPDF');
            var n = file.indexOf("?");
            if(n > 0){
                var char = '&';
            }else{
            ...
            changeSrcEmbed(element,file+char+'q=cat&' + oDate.getTime());
            
            //$( "#myModalLabel" ).text( title);
            $('#modalLectorPDF').modal('show');
}
