<?php
/*
	Classe para conex�o em PostgreSQL, para utiliz�-lo renomeie este arquivo
	para "db.class.php"
*/
...
class db {
	...
	function __construct($database_type="postgresql") {
	...
	//----- envia commit ao servidor de dados
        function setDB() {
		pg_query($this->connect_id, "SET search_path = admin, catalogos, personal, portal, gestdoc, gestleg, 
                                                               gestmed, farmacia, serfin, siscore, audit_log, sispat,
                                                               cementerio, siscopp, sislogal, siscont, solicitudes, 
                                                               gestcne, alquileres, sisteso, public;
	...
	function nextid($sequence='') {
		...
		$esequence=str_replace("'","''",$sequence);
                
		$queryx=new query($this, "select nextval('$esequence') as nextid") ;
		if ($queryx->numrows()) {
			$queryx->getrow()	;	
			$nextid=$queryx->field("nextid");
		} else {
			$queryx=new query($this, "create sequence $sequence")  ;
			if ($queryx->result) {
		...
		if ($this->result) {
			$this->free();
                ...
                if (($pagina_inicial+$tamanho_pagina) > 0) {
                    $this->result=@pg_query($db->connect_id, "SELECT COUNT(*) AS num_rows FROM ($query) AS a ");
                    $this->numrows = @pg_fetch_result($this->result, 0, 0);
